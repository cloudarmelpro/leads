...
/**
 * Page Prix (maquette « Pricing v3 ») : barre collante des familles, cartes de gamme
 * (Site web) ou de forfait (Logo, Hébergement, VPS), puis comparaison détaillée en
 * tableau dès 900px et en cartes à onglets en dessous. Un seul état : famille, gamme,
...
export function PricingExplorer({ lang, dict }: Props) {
  ...
  const groupKey = (title: string) => `${family.key}|${title}`;

  return (
    <>
      {/* Barre des familles : collante, fond translucide flouté, sans filet. */}
      ...
      <section className={`flex justify-center pt-[64px] pb-[clamp(112px,16vw,240px)] ${GOUTTIERE}`}>
        <div className="flex w-full max-w-[1100px] flex-col gap-[48px]">
          {/* En-tête : label + titre de la famille, description alignée à droite. */}
          <div className="flex flex-wrap items-end justify-between gap-[40px]">
            <div className="flex flex-col gap-[2px]">
              ...
              <h2 className="m-[0px] text-[clamp(22px,2.6vw,30px)] leading-[1.15] font-normal tracking-[-0.4px] text-encre">{family.title}</h2>
            </div>
            <p className="m-[0px] ml-auto max-w-[420px] text-right text-[14px] leading-[24px] font-normal text-texte2 text-pretty">{family.desc}</p>
          </div>
          ...
          {hasRanges ? (
            <div className="grid grid-cols-[repeat(auto-fit,minmax(240px,1fr))] gap-[16px]">
              {family.ranges.map((r, i) => {
                ...
                const lit = active || hover === `g${i}`;
                const select = () => {
                  setRange(i);
                  setPick(0);
                };
                return (
                  <div key={r.title} className="flex flex-col gap-[10px]">
                    <button
                      type="button"
                      aria-pressed={active}
                      onClick={select}
                      onMouseEnter={() => setHover(`g${i}`)}
                      onMouseLeave={() => setHover((h) => (h === `g${i}` ? null : h))}
                    ...
                    >
                      ...
                      <span className={`text-[14px] leading-[21px] font-normal text-pretty ${active ? "text-white/78" : lit ? "text-white/78" : "text-texte2"}`}>{r.line}</span>
