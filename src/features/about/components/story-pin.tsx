...
/**
 * Notre histoire (maquette À propos, sans l'épinglage : le client l'a retiré le
 * 2026-09-25) : la citation s'allume mot à mot à l'approche, les trois volets sont
 * visibles et un clic en ouvre un. Le bloc bascule en 3D à l'entrée et s'estompe en
 * sortant, calé sur le défilement ; rien ne bouge avec moins d'animations.
 */
export function StoryPin({ quote, items }: Props) {
  ...
  useEffect(() => {
    ...
    const tick = () => {
      ...
      const lp = reduce || !lit ? 1 : clamp01((vh * 0.9 - lit.getBoundingClientRect().top) / (vh * 0.65));
    ...
    // Entrée en 3D (sur la section) et sortie en fondu (sur le contenu) : deux éléments
    // distincts, sinon un recalcul de ScrollTrigger pouvait figer un flou en cours. Le
    // filtre est retiré dès la sortie revenue à zéro pour garder le texte net.
    const ctx = gsap.context(() => {
      if (reduce) return;
      gsap.fromTo(
        el,
        { rotationX: 10, y: 40, opacity: 0.75, transformPerspective: 1200, transformOrigin: "50% 100%" },
        { rotationX: 0, y: 0, opacity: 1, ease: "none", immediateRender: true, scrollTrigger: { trigger: el, start: "top bottom", end: "center 55%", scrub: 0.5 } },
      );
      gsap.fromTo(
        content,
        { y: 0, opacity: 1, filter: "blur(0px)" },
        {
          ...
          scrollTrigger: { trigger: el, start: "center 40%", end: "bottom top", scrub: 0.5 },
      ...
      if (mark.current) {
        gsap.fromTo(mark.current, { rotate: -12 }, { rotate: 0, ease: "none", scrollTrigger: { trigger: el, start: "top bottom", end: "top 40%", scrub: true } });
      }
    });
    return () => {
  ...
  return (
    <section ref={section} id="histoire" className="relative flex justify-center px-[clamp(16px,4vw,56px)] py-[clamp(48px,6vw,96px)]">
        ...
        <div className="flex flex-col gap-[14px]">
          {items.map((item, i) => {
            ...
            return (
              <div
                key={item.title}
                className={`relative flex flex-col overflow-hidden rounded-[16px] px-[clamp(20px,2.4vw,32px)] py-[clamp(20px,2vw,28px)] transition-[background] duration-[800ms] ease-[cubic-bezier(0.65,0,0.35,1)] ${
                  open
