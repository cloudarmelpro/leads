...
const SPARK = (
  <svg viewBox="0 0 24 24" aria-hidden className="size-[26px] shrink-0 [filter:drop-shadow(0_0_10px_rgba(127,239,192,0.9))]">
    <path d="M12 0C12.9 8 16 11.1 24 12C16 12.9 12.9 16 12 24C11.1 16 8 12.9 0 12C8 11.1 11.1 8 12 0Z" fill="#7FEFC0" />
...
/**
 * Bandeau au bas du hero À propos : les quatre phrases défilent en continu (liste répétée
 * trois fois, décalée d'un tiers pour boucler sans saut), et deux faisceaux de lumière
 * balaient le bandeau toutes les 7 s. Immobile sous `prefers-reduced-motion`.
 */
export function HeroBand({ items, label }: Props) {
  ...
  return (
    <div aria-label={label} className="absolute inset-x-[0px] bottom-[0px] z-[3] flex items-center overflow-hidden py-[18px]">
      ...
      <div ref={band} className="flex w-max will-change-transform">
        {list.map((text, i) => (
          <span key={i} aria-hidden={i >= items.length || undefined} className="flex shrink-0 items-center gap-[clamp(24px,2.8vw,40px)] pr-[clamp(24px,2.8vw,40px)]">
            <span className="text-[clamp(22px,15.79px+1.66vw,36px)] leading-[1.08] font-semibold tracking-[-1px] whitespace-nowrap text-[#30D98C] uppercase min-[620px]:tracking-[-2px]">{text}</span>
            {SPARK}
          </span>
        ))}
      </div>
